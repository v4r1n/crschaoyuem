...
- OAuth does not rely on `Session.getTemporaryActiveUserKey()` as identity or a required binding because anonymous Pilot RPCs can return it empty. Flow ownership uses separate browser-held poll/session proofs; verified Google identity plus the Users row remains the identity/authorization source. Callback state, PKCE, nonce, OTP and current Users checks remain mandatory.
...
โปรเจกต์ Apps Script แบบ standalone เชื่อม Google Sheet, Drive folder และ Web OAuth Client ID/secret ด้วย Script Properties Production คง versioned Web app แบบ `USER_DEPLOYING` + `ANYONE`; Pilot อาจใช้ `ANYONE_ANONYMOUS` เพื่อโหลด shell ก่อน OAuth แต่ไม่มีสิทธิ์ business RPC จนกว่าจะตรวจ Google identity, session proof และ Users row สำเร็จ Backend จึงใช้สิทธิ์ผู้ deploy โดยผู้ใช้ทั่วไปไม่มี direct access ต่อ Sheet/folder

OAuth Client ต้องเป็นชนิด **Web application** และลงทะเบียน Authorized redirect URI แบบ exact เป็น `https://script.google.com/macros/s/{PILOT_DEPLOYMENT_ID}/exec`; flow นี้ไม่ใช้ Authorized JavaScript Origin และไม่โหลด GIS JavaScript จาก iframe การแก้ deployment เดิมให้ชี้ version ใหม่จะรักษา `/exec` URL และ QR sticker เดิม แต่ต้องทดสอบ Pilot callback/proof binding กับทั้ง Workspace และ Gmail ก่อนเปลี่ยน production รายละเอียดอยู่ใน [DEPLOYMENT.md](DEPLOYMENT.md)
...
Neither the callback nor visitor RPCs read or compare `getTemporaryActiveUserKey()`; anonymous Apps Script RPCs may return it empty. Flow ownership relies on the independent browser-held poll/session proofs, with callback state one-time and TTL-bound. No Google/session token appears in URLs. Raw callback state is hashed in cache; nonce/PKCE verifier are transient server-only cache data. Random server values use a domain-separated HMAC-SHA256 PRF keyed by the confidential OAuth client secret with UUID/time uniqueness input; protect/rotate that secret and never log it.

Pilot deployment procedure: pass all tests, commit and push `main`, verify `USER_DEPLOYING` + `ANYONE_ANONYMOUS`, upload, create an immutable version, download/compare the new immutable version, then update ONLY the existing Pilot deployment. No separate pre-deploy backup is created. Production promotion requires real YRU/Gmail login, confirmation, session isolation, and authorization acceptance.
