...
The anonymous-login regression is fixed in `OAuthService.gs`: OAuth/session ownership no longer depends on `Session.getTemporaryActiveUserKey()`. Contract, backend, and Playwright suites pass 79/79 on 2026-09-24; `git diff --check` is clean. Next: commit and push this source, push it to the existing Apps Script project, create one immutable release version (not a backup), download/compare that version, then update only the existing Pilot deployment as `USER_DEPLOYING` + `ANYONE_ANONYMOUS`; keep Production unchanged and perform live Workspace/Gmail sign-in acceptance.
